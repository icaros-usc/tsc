...
class TransitionStateClustering:
    ...
    def taskToTrajectory(self):
        ...
        for i in range(0, N):
            ...
            tseg.append(0)
            tseg.append(int(self._demonstration_sizes[i][0] - self.window_size))
    ...
    def clusterInState(self):
        ...
        for i in range(0, tsN):
            ...
            ts_data_array[i, :] = self._demonstrations[ts[0]][ts[1], :, 0]
        ...
        if self.verbose:
            print("[TSC] Discovered State Clusters (demoid, time, statecluster): ",
                  self._transition_states_scluster)
    ...
    def clusterInTime(self):
        ...
        for i in range(0, self._distinct_state_clusters):
            ...
            for j in range(0, len(tsI)):
                ts = tsI[j]
                ts_data_array[j, :] = self._demonstrations[ts[0]][ts[1], :, 0]
            ...
            mm = mixture.GaussianMixture(n_components=1)
    ...
    def compaction(self, delta=-1):
        for i in range(0, len(self._demonstrations)):
            ...
            for j in range(0, len(segs) - 1):
                ...
                if prev is not None and len(cur) > 0 and len(prev) > 0:
